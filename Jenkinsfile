pipeline {
    ...
    stages {
        ...
        stage('Publish') {
            when { branch 'main' }
            steps {
                sh "docker tag ${GIT_COMMIT} fintlabsacr.azurecr.io/fint-skjema-acos-instance-gateway:build.${BUILD_NUMBER}_${GIT_COMMIT}"
                withDockerRegistry([credentialsId: 'fintlabsacr.azurecr.io', url: 'https://fintlabsacr.azurecr.io']) {
                    sh "docker push fintlabsacr.azurecr.io/fint-skjema-acos-instance-gateway:build.${BUILD_NUMBER}_${GIT_COMMIT}"
                }
                /**
